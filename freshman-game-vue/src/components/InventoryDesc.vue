    ...
    export default {
        ...
        data(){
            return({
                ...
                item_desc:[" ", "대학생은 강하다. 개강하기 때문이다.", "주로 금요일에 있다. 퀴즈를 잘 보면 그날은 기분이 좋더라.", "진짜 엄청 무겁다. 태블릿 사는 것을 추천.  ", "안 듣는 것이 낫다, 후회할 가능성이 높다.", "짐을 들고 오르내리다 보면 전완근이 아려 오기 시작한다.", "조교님이 진행하시는 연습반. 수업에 모르는 부분이 있었다면 연습반 시간에 물어보자.", "귀찮긴 하지만 이거 없으면 학기 중에 공부 1도 안하니까 열심히 하자.", "원래대로라면 직접 실험을 했겠지만 비대면으로 인해 보고서만 작성하고 제출하면 된다.", "성실한 학생이라면 방학에도 계절학기를 들어 학점을 채울 수 있다.", "새내기가 치르는 첫 대학 시험. 모두 화이팅!", "머리가 아플 수도 있지만 방학 생각 하나로 버틴다.", "매 학기 초 예습, 복습 계획을 세우지만 번번히 실패하곤 한다.", "점심 먹고 강의를 듣다 보면 스르륵 감기는 눈꺼풀...", "이건 못 참지!", "공짜로 받을 수 있는 기초과목 과외. 부담되는 과목이 있다면 신청하도록 하자.", "족발보쌈....이 아니라 대대로 내려온 시험지 등. 알차게 활용하고 물려주도록 하자.", "갑자기 생긴 공강만큼 기분 좋은 게 없다.", "잔디밭에 돗자리를 펴 놓고 오순도순 딸기를 먹는 파티.... 였던 것.", "나도 궁금하다(지나가던 20학번)", "자다가 수업을 결석한 날이면 이 과목에 라이프가 있었기를 기도하곤 한다.", "빠드익선", "이걸 입으면 진짜 대학생이 된 기분이 든다.", "단돈 5만원으로 겨울을 따뜻하게 보낼 수 있다.", "무난한 청바지", "평범한 카고바지. 주머니가 많지만 실제로 물건을 넣지는 않는다.", "공대생이라면 체크남방 하나쯤은 있어야지?", "교내에서 편하게 입기 딱 좋은 후드티."],
                id1_to_att:[0,2,4,4,4,8,8,8,8,12,12,16],
                id1_to_range:[0,3,4,2,3,1,2,3,3,3,2,2],
                id12_to_def:[3,4,3,2,6,7,5,5,10,14,0,0,0,0,0,0],
        ...
        methods:{
            get_src(id){
                try{
                    return `https://storage.googleapis.com/kaiground/assets/ItemImage/${id}.png`
                } catch(e) {
